...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.btn-remove-image')) {
            ...
            if (imageId) {
                ...
                removedImagesInput.value = currentRemoved ? `${currentRemoved},${imageId}` : imageId;
                imageContainer.remove();
                
                // Hide gallery if empty
                const imageGallery = document.getElementById('image_gallery');
                if (!imageGallery.children.length) {
                    document.getElementById('gallery_container').classList.add('d-none');
                }
            }
        }
    ...
    function handleFormSubmit(e) {
        ...
        .then(data => {
            if (data.success) {
                ...
                Toast.fire({
                    icon: 'success',
                    title: data.message || 'Car saved successfully'
                });
            } else {
    ...
    function validateForm() {
        ...
        // Check assurance_expiry_date
        const assuranceExpiryDate = document.getElementById('assurance_expiry_date');
        if (assuranceExpiryDate && assuranceExpiryDate.value) {
            const date = new Date(assuranceExpiryDate.value);
            if (date < today) {
                assuranceExpiryDate.classList.add('is-invalid');
                const errorElement = document.getElementById('assurance_expiry_date-error');
                if (errorElement) {
        ...
        // Check vignette_expiry_date
        const vignetteExpiryDate = document.getElementById('vignette_expiry_date');
        if (vignetteExpiryDate && vignetteExpiryDate.value) {
            const date = new Date(vignetteExpiryDate.value);
            if (date < today) {
                vignetteExpiryDate.classList.add('is-invalid');
                const errorElement = document.getElementById('vignette_expiry_date-error');
                if (errorElement) {
                    errorElement.textContent = 'Expiry date must be in the future';
                    errorElement.style.display = 'block';
                }
        ...
        // Check visite_technique_expiry_date
        const visiteExpiryDate = document.getElementById('visite_technique_expiry_date');
        if (visiteExpiryDate && visiteExpiryDate.value) {
            const date = new Date(visiteExpiryDate.value);
            if (date < today) {
                visiteExpiryDate.classList.add('is-invalid');
                const errorElement = document.getElementById('visite_technique_expiry_date-error');
                if (errorElement) {
                    errorElement.textContent = 'Expiry date must be in the future';
                    errorElement.style.display = 'block';
                }
        ...
        // If not valid, show first tab with errors
        if (!isValid) {
            const firstInvalidElement = document.querySelector('#carForm .is-invalid');
    ...
    function handleEditCar(carId) {
        ...
        .then(data => {
            ...
            if (data.success && data.car) {
                ...
                // Set document fields
                if (car.documents) {
                    document.getElementById('assurance_number').value = car.documents.assurance_number || '';
                    document.getElementById('assurance_company').value = car.documents.assurance_company || '';
                    document.getElementById('carte_grise_number').value = car.documents.carte_grise_number || '';
                    
                    // Format date fields
                    if (car.documents.assurance_expiry_date) {
                        document.getElementById('assurance_expiry_date').value = formatDate(car.documents.assurance_expiry_date);
                    }
                    if (car.documents.carte_grise_expiry_date) {
                        document.getElementById('carte_grise_expiry_date').value = formatDate(car.documents.carte_grise_expiry_date);
                    }
                    if (car.documents.vignette_expiry_date) {
                        document.getElementById('vignette_expiry_date').value = formatDate(car.documents.vignette_expiry_date);
                    }
                    if (car.documents.visite_technique_date) {
                        document.getElementById('visite_technique_date').value = formatDate(car.documents.visite_technique_date);
                    }
                    if (car.documents.visite_technique_expiry_date) {
                        document.getElementById('visite_technique_expiry_date').value = formatDate(car.documents.visite_technique_expiry_date);
                    }
                    
                    // Show document file indicators
                    if (car.documents.file_carte_grise) {
                        document.getElementById('carte_grise_file_indicator').classList.remove('d-none');
                        document.getElementById('carte_grise_file_indicator').href = `/storage/${car.documents.file_carte_grise}`;
                    }
                    if (car.documents.file_assurance) {
                        document.getElementById('assurance_file_indicator').classList.remove('d-none');
                        document.getElementById('assurance_file_indicator').href = `/storage/${car.documents.file_assurance}`;
                    }
                    if (car.documents.file_visite_technique) {
                        document.getElementById('visite_technique_file_indicator').classList.remove('d-none');
                        document.getElementById('visite_technique_file_indicator').href = `/storage/${car.documents.file_visite_technique}`;
                    }
                    if (car.documents.file_vignette) {
                        document.getElementById('vignette_file_indicator').classList.remove('d-none');
                        document.getElementById('vignette_file_indicator').href = `/storage/${car.documents.file_vignette}`;
                    }
                }
                ...
                if (car.main_image) {
                    ...
                    imagePreview.querySelector('img').src = mainImageUrl;
                }
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
                            text: data.message || 'Car deleted successfully',
                            confirmButtonColor: '#5e72e4'
                        });
                    } else {
    ...
    document.getElementById('update_documents_btn')?.addEventListener('click', function() {
        ...
        if (!carId) {
            Swal.fire({
                icon: 'warning',
                title: 'Warning',
                text: 'Please save the car first before updating documents',
                confirmButtonColor: '#5e72e4'
            });
            return;
        }
    ...
    function updateDocuments(carId) {
        ...
        const fileCarteGrise = document.getElementById('file_carte_grise').files[0];
        const fileAssurance = document.getElementById('file_assurance').files[0];
        const fileVisiteTechnique = document.getElementById('file_visite_technique').files[0];
        const fileVignette = document.getElementById('file_vignette').files[0];
        
        if (fileCarteGrise) formData.append('file_carte_grise', fileCarteGrise);
        if (fileAssurance) formData.append('file_assurance', fileAssurance);
        if (fileVisiteTechnique) formData.append('file_visite_technique', fileVisiteTechnique);
        if (fileVignette) formData.append('file_vignette', fileVignette);
        ...
        .then(data => {
            if (data.success) {
                ...
                // Clear file inputs
                document.getElementById('file_carte_grise').value = '';
                document.getElementById('file_assurance').value = '';
                document.getElementById('file_visite_technique').value = '';
                document.getElementById('file_vignette').value = '';
            } else {
        ...
        .catch(error => {
            ...
            if (error.status === 422 && error.data && error.data.errors) {
                ...
                for (const field in error.data.errors) {
                    ...
                    if (errorElement && error.data.errors[field][0]) {
    ...
    // Make handleEditCar function globally available for use in HTML
